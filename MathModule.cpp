class ShaderNodeMathModule : public Module {
  ...
  override auto GenerateTokenString(): AbstractToken[] {
    ...
    if (op == "ADD") {
      expr = { WildcardToken("Value0"), TextToken(" + "), WildcardToken("Value1") };
    }
    else if (op == "SUBTRACT") {
      expr = { WildcardToken("Value0"), TextToken(" - "), WildcardToken("Value1") };
    }
      ...
      expr = { WildcardToken("Value0"), TextToken(" * "), WildcardToken("Value1"), TextToken(" + "), WildcardToken("Value2") };
    }
    // Power & Logarithmic
    else if (op == "POWER") {
      expr = { TextToken("pow("), WildcardToken("Value0"), TextToken(", "), WildcardToken("Value1"), TextToken(")") };
    }
    else if (op == "LOGARITHM") {
      expr = { TextToken("log("), WildcardToken("Value0"), TextToken(") / log("), WildcardToken("Value1"), TextToken(")") };
    }
    else if (op == "SQRT") {
      expr = { TextToken("sqrt("), WildcardToken("Value0"), TextToken(")") };
    }
    else if (op == "INVERSE_SQRT") {
      expr = { TextToken("inversesqrt("), WildcardToken("Value0"), TextToken(")") };
    }
    else if (op == "EXPONENT") {
    ...
    // Comparison & Min/Max
      ...
      expr = { TextToken("min("), WildcardToken("Value0"), TextToken(", "), WildcardToken("Value1"), TextToken(")") };
    }
    else if (op == "MAXIMUM") {
      expr = { TextToken("max("), WildcardToken("Value0"), TextToken(", "), WildcardToken("Value1"), TextToken(")") };
    }
    else if (op == "LESS_THAN") {
      expr = { TextToken("float("), WildcardToken("Value0"), TextToken(" < "), WildcardToken("Value1"), TextToken(")") };
    }
    else if (op == "GREATER_THAN") {
      expr = { TextToken("float("), WildcardToken("Value0"), TextToken(" > "), WildcardToken("Value1"), TextToken(")") };
    }
    else if (op == "COMPARE") {
      expr = { TextToken("float(abs("), WildcardToken("Value0"), TextToken(" - "), WildcardToken("Value1"), TextToken(") <= "), WildcardToken("Value2"), TextToken(")") };
    }
    // Rounding & Modulo
    else if (op == "ABSOLUTE") {
      ...
      expr = { TextToken("floor("), WildcardToken("Value0"), TextToken(" + 0.5)") };
    }
    else if (op == "MODULO" || op == "FLOORED_MODULO") {
      expr = { TextToken("mod("), WildcardToken("Value0"), TextToken(", "), WildcardToken("Value1"), TextToken(")") };
    }
    else if (op == "SNAP") {
      expr = { TextToken("floor("), WildcardToken("Value0"), TextToken(" / "), WildcardToken("Value1"), TextToken(") * "), WildcardToken("Value1") };
    }
    // Trigonometry
    else if (op == "SINE") {
      ...
      expr = { TextToken("atan("), WildcardToken("Value0"), TextToken(", "), WildcardToken("Value1"), TextToken(")") };
    }
    else if (op == "SINH") {
